...
void integration_first_order_adapt(double final_time, double tau, int nsteps_int, const grid_info<3>& gi, vector<const double*> X0, vector<const double*> V0, double tol1, double tol2, Index min_r, Index max_r, Index snapshots, const blas_ops& blas){
...
int main(int argc, char** argv){
  ...
  ("nx", "Number of grid points in space (as a whitespace separated list)", cxxopts::value<string>()->default_value("4 4 4"))
  ("nv", "Number of grid points in velocity (as a whitespace separated list)", cxxopts::value<string>()->default_value("4 4 4"))
  ...
  ("r_min", "Minimum rank of the simulation", cxxopts::value<int>()->default_value("5"))
  ...
  ("tol", "Tolerance for error control", cxxopts::value<double>()->default_value("0.00001"))
  ...
  double  tol1 = result["tol"].as<double>(); // tolerance to increase rank
  double  tol2 = result["tol"].as<double>(); // tolerance to decrease rank
